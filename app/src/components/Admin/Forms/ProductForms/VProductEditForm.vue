...
export default {
  ...
  setup(props, { emit }) {
    ...
    const { selectedProduct, materials, furnitures, categories, sizes, cuts } = toRefs(props);
    ...
    onUpdated(() => {
      ...
      state.materials = selectedProduct.value.materials.map((m) => m.material.name);
      state.furnitures = selectedProduct.value.furnitures.map((f) => f.furniture.name);
      ...
      if (result) {
        store
          .dispatch("product/UPDATE_PRODUCT", {
            ...
            image: state.image[0],
            categoryId: categories.value.rows.filter((i) => i.name === state.category)[0]
              .id,
