<template>
  <v-card>
    <v-card-title class="border-b my-3 text-center font-weight-light text-h4"
      >Добавить новую фурнитуру</v-card-title
    >
    <v-card-text>
      <v-sheet class="my-3 w-100">
        <v-form @submit.prevent="addNewFurniture">
          <v-text-field
            v-model="state.name"
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const { furnitureColors } = toRefs(props);
    ...
    const addNewFurniture = async () => {
      console.log(state.image[0]);
      const result = await v$.value.$validate();
      if (result) {
        store
          .dispatch("furniture/ADD_FURNITURE", {
            name: state.name,
            price: state.price,
            count: state.count,
